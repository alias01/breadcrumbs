...
- Step 3: one wrap-up message after all tasks, not per-task — but it's still the review gate (`step3-implement.md` 3.4): feedback → fix, re-show, loop until none, same as full mode just collapsed to one round instead of one per task.
...
| 3 — Implement | `step3-implement.md` | Every task reviewed (fix/feedback loop until none) and checked off |
