...
- **Stop signal** — "let's continue tomorrow," "pause here," or similar → create now, backfill Original Story/Understanding/Plan/Task Checklist from the conversation, at whatever step you're at. Mode/design depth unchanged — this trigger alone doesn't escalate lite → full. Story stops for an external cause or is dropped outright ("blocked on the platform team," "we're not doing this") → same creation, then the parked status and its reason ("Parking a story" in `context-file-mechanics.md`).
...
On creation, write the header block in full: title, `Status:` at whatever step you're at, and `Branch:` from `git rev-parse --abbrev-ref HEAD` (omit outside a git repo). All three are first-two-lines content the resume scan and staleness sweep below read without opening the rest of the file — cheap only because they're there.
...
**Resuming:** before any story work, check `.breadcrumbs/context/` for existing files. One match, name/slug clearly matches what the user's asking about → read it, summarize status back ("Here's where this stood: ... currently at Step X"), resume. **Check `Branch` against `git rev-parse --abbrev-ref HEAD` as part of that read** — mismatch → say so before resuming ("file says `feature/csv-export`, you're on `main`") and ask whether to switch. Never resume silently onto a different branch: the Task Log would then record commits that landed somewhere the file doesn't name. No `Branch` line (file predates it, or no git) → note it once, write the current branch in on the next gate write. Zero matches → nothing to resume; story hasn't started, or it's mid-way/finished in an unbroken conversation with no trigger fired yet. More than one file present and the user's request doesn't unambiguously point to one (generic "let's continue," or a new/vague prompt while other stories sit mid-flight) → don't guess. List the candidates cheaply: filename (slug) + first two lines of each (title, `Status:`) — never a full read at this stage, cost shouldn't scale with how many stories are open or how long they've grown. Present that list, ask which one. Once picked, proceed as the one-match case (full read, then resume).

**Staleness check (piggybacks on the scan above, no separate pass):** the directory listing itself — filenames + mtimes — is already free at this point regardless of match count. While scanning, read each file's first two lines (same cheap read as the listing case) and flag these combinations of `Status:` + mtime:

| Status | Older than | Means | Ask |
|---|---|---|---|
| `pr-ready` | 7 days | PR draft went out, nobody confirmed merge | "merged? want these deleted?" |
| `blocked` | 14 days | parked on an external cause that may have cleared | "still blocked?" |
| `implementing` | 30 days | nobody stopped it and nobody continued it — silent rot | "still live, or should this be `abandoned`?" |

`abandoned` and `done` are settled end states — never swept, they're already the answer. The 30-day `implementing` row is the one that catches a story nobody ever declared dead; without it, a forgotten story is indistinguishable from an active one and just accumulates as another candidate in the disambiguation list above.

Collect across the whole directory, not just the story being resumed or started. Any found → after resolving the current story's resume/start, mention them once in one line, grouped: "N context file(s) stale: <slug> (pr-ready 12d), <slug> (implementing 40d) — merged / still live / delete?" Confirmed per-file or in bulk → delete. Same rule as ordinary cleanup: never delete unprompted, this only surfaces the offer sooner instead of waiting for someone to reopen that specific story.
...
**Cleanup:** PR merged (user-confirmed) → offer to delete. Never delete unprompted. Declined ("keep it," "we commit these") → set `Status: done` instead, same write. That's the one thing that writes `done`: a file kept on purpose past the merge it was working memory for. Deletion stays the default — `done` is not a step every story passes through on its way out.

**Parking a story** — work stops without reaching a PR. Two states, both set on an explicit user signal, never inferred from silence:

- `blocked` — can't proceed, external cause (another team, an unmerged dependency, a decision that hasn't come back). Story is expected to resume.
- `abandoned` — deliberately stopped. Deprioritized, superseded, or the requirement was dropped outright.

Either one: flip `Status` in place **and** write a Scope Changes entry in the same pass — trigger, before/after, why, and for `blocked` what would unblock it. The status says *that* the story stopped; only the entry says *why*, and a story that dies with no reason recorded is the exact failure this file exists to prevent. Deleting an abandoned story throws that reasoning away, which is why `abandoned` exists rather than "just delete it" — the record of a decision not to build something is worth as much as the record of building it.

Both are also file-creation triggers when no file exists yet (same handling as a Stop signal): create, backfill, then set the parked status. Resuming a `blocked` story later → normal resume, flip `Status` back to whatever step it stopped at, note in one line that the block cleared.
...
3. Fixed section set, same five names regardless of type — **What, Why, Test, Rollback, Dependencies.** No type-varying sections anymore. Bias toward fewer: include a section only if it earns its place for a reviewer, omit rather than pad. File exists → pull content from it, don't re-summarize; no file → derive the same from the conversation.
   ...
   - **Dependencies** — only if this PR depends on or blocks something else, including sitting on top of another unmerged branch. ← Assumptions/Plan, only where flagged; branch dependency ← `git merge-base HEAD <default-branch>` isn't `<default-branch>`'s tip → this branch is stacked, name the base branch, note it needs merging first. Confirm `HEAD` matches the file's `Branch:` line before reading anything into the comparison — run it on the wrong branch and the answer is about someone else's work.
