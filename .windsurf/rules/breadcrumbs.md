...
Auto-detected in Step 1.4: `Bug fix` and `Copy/config/content change` stories run lite; everything else runs full mode, exactly as the four steps below describe. This section is the single source of truth for what lite changes — the steps below aren't separately annotated for it.

- No context file, ever.
- Step 1's gate and all of Step 2 collapse into one: state the approach (always "no design" depth) and task list (max 2), then one message ending "Here's what I understand and how I'd build it — confirm?" Straight to Step 3 once confirmed.
- Step 3: no per-task writes — one wrap-up message after all tasks instead.
- Step 4: PR draft as usual (same sections, same 2-line cap), derived from the conversation instead of the file; nothing to write after.
- Upgrade path: a test failure, broken assumption, or scope change mid-flight ends lite mode — create the file now, backfilling Original Story/Understanding/Plan/Task Checklist from the conversation, log the Scope Change entry (Step 3's usual format), and finish the story in full mode. Say so in one line.
...
4. Classify the story type now, using the table in Step 2.1 — don't wait for Step 2. `Bug fix` and `Copy/config/content change` are **lite** (see "Lite mode" above); everything else is **full**. Say which mode out loud (one line) so the user isn't surprised later.
5. **Gate:** write the Understanding Summary and Assumptions to the context file in one pass (see Efficiency above — this is also where the first-use `.gitignore` check/add happens). Emit the trip marker, then present the summary — quote it verbatim rather than restating it — and stop. Do not move to Step 2 until the user explicitly confirms it's right.

### Step 2 — Plan

*Lite-mode stories skip this step* — see "Lite mode" above.
...
4. After each task, append a Task Log entry and check off the task in the same write, emit the trip marker, then tell the user what was done, then move to the next task without waiting for a response, unless the user interjects. Use the full What/Why block only for tasks where a genuine judgment call was made (per 3.2); log purely mechanical tasks as a single checklist line instead — see `context-template.md` for both forms. Quote whichever form you wrote rather than restating it.
5. If a test fails, the owner clarifies something that invalidates an assumption, or scope changes mid-flight: treat it as a fresh mini-cycle — add a structured entry under Scope Changes (date, trigger, before/after, affected tasks, why), amend Current Requirements in place to reflect the new state, update the relevant Assumptions/Plan sections, adjust the Task Checklist (add/modify tasks) rather than starting a new file, and continue.
