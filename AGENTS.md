...
1. Work the Task Checklist one task at a time. **Zero chat output per task — not even an end-of-task message.** Edit, verify, task-log write all run back to back in silence: no lead-in ("starting task N"), no running commentary ("verifying now", "typecheck clean"), no end-of-task summary either. Nothing is committed here either — see point 6. This isn't deferred narration for its own sake — it's deferred to a *review*, not a report: point 8's gate presents the whole task list, with every task's actual pending diff still sitting uncommitted, for the user to check *before* any of it is committed — which is where "what changed and why" actually needs to land for the user to catch deviation, not scattered across turns they'd have to remember to compare against a diff later, and not after it's already locked into history. The one exception is point 7 — a test failure, invalidated assumption, scope change, or perf/scale regression is flagged the moment it's found, in real time, never held for the review.
...
8. **Gate:** every task checked off → first, one honest check on yourself: any chat line between point 1's start and here that wasn't a point-7 break ("Task N verified", "moving to Task N+1", "typecheck clean") means point 1 was broken — say so in one line at the top of the review, don't repeat it next story. Then **run the whole planned test set now**: every case Step 2.4 listed (lite → the check named at the collapsed gate), plus the repo's own suite over what the story touched, against the still-uncommitted working tree. Per-task runs prove tasks in isolation; this proves they compose.
