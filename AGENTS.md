...
8. **Gate:** every task checked off → **run the whole planned test set now**: every case Step 2.4 listed (lite → the check named at the collapsed gate), plus the repo's own suite over what the story touched, against the still-uncommitted working tree. Per-task runs prove tasks in isolation; this proves they compose.
