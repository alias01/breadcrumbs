...
- **hooks:** `guard-stash.mjs` denies `git stash`/`checkout --`/`restore`/`revert` — the regression-test-first rule held in one of three runs as prose; measured in docs/decisions.md
