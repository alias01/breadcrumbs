...
10. **Gate:** full mode → budget checkpoint write first ("Budget checkpoint" in `context-file-mechanics.md`: file created if needed, `Status: planning`). Then investigation marker (see "Investigation scope" in `SKILL.md`), then trip marker. Present plan + task breakdown, quoted verbatim, closing with the `[checkpoint: …]` line. Stop, wait for confirmation before implementing (`step3-implement.md`).
