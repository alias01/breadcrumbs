...
8. **[Testing] Session token stats.** Run `session-token-stats.mjs --by-tool` (resolve per "Validator scripts" in `context-file-mechanics.md` — don't assume `scripts/` is relative to the target repo's cwd) and show the full breakdown table under the draft, not just the total — the point of this step is spotting which category (system prompt, Read, Bash, Edit, chat replies, ...) is driving cost, which the one-line total can't show. Not found → skip, don't block. Drop this step once it's no longer needed for testing.
